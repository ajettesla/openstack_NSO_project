...
if [ -z "$number" ]; then
    echo "No number found in configuration file!"
    exit 1
fi
...
echo "Desired number of servers: $number"

# Get the current number of servers with "dev" in their name
nofsr=$(openstack server list -f csv | grep "dev" | wc -l)
echo "Number of servers running is $nofsr"

# Check if the number of servers needs adjustment
if [ "$number" -lt "$nofsr" ]; then
    # Calculate how many more servers are needed
    check=$(($nofsr - $number))
    ...
    echo "Number of servers running and required servers are equal $number"
    exit 1
fi
